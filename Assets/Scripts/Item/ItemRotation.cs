...
public class ItemRotation : MonoBehaviour {
    ...
    public void RefreshUI() {
        if(rightCollisionSprite && WorldManager.instance.worldMapTile[(int)this.transform.position.x + 1, (int)this.transform.position.y] > 0) {
            this.renderer.sprite = this.rightCollisionSprite;
            this.collisionSide = Direction.RIGHT;
        } else if(leftCollisionSprite && WorldManager.instance.worldMapTile[(int)this.transform.position.x - 1, (int)this.transform.position.y] > 0) {
            this.renderer.sprite = this.leftCollisionSprite;
            this.collisionSide = Direction.LEFT;
        } else if(topCollisionSprite && WorldManager.instance.worldMapTile[(int)this.transform.position.x, (int)this.transform.position.y + 1] > 0) {
            this.renderer.sprite = this.topCollisionSprite;
            this.collisionSide = Direction.TOP;
        } else if(bottomCollisionSprite && WorldManager.instance.worldMapTile[(int)this.transform.position.x, (int)this.transform.position.y - 1] > 0) {
