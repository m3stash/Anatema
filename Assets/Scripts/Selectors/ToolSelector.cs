...
{
    [Header("Fields to complete")] [SerializeField]
    private DigSelector digSelectorPrefab;

    [SerializeField] private GameObject digParticle;

    [Tooltip("Fill with all items id which are not allowed to be destroyed over the tile")] [SerializeField]
    private int[] notAllowedItemIds;

    [Tooltip("Fill with all items id which are allowed to be destroyed by tile on horizontal Axis")] [SerializeField]
    private int[] allowedItemsIds;

    [Header("Don't touch it")] [SerializeField]
    private ToolType currentToolType = ToolType.PICKAXE;
    ...
    private void OnEnable()
    {
        ...
        if (InputManager.gameplayControls != null)
        {
            // TODO remove that after orchestrator
    ...
    void Update()
    {
        ...
        // Manage detections for each tools
        if (this.currentToolType == ToolType.PICKAXE)
        {
            this.ManagePickaxeDetection();
        }
        else if (this.currentToolType == ToolType.AXE)
        {
            this.ManageAxeSystemDetection();
        }
        else if (this.currentToolType == ToolType.HAMMER)
        {
            this.ManageHammerDetection();
        }
    }

    private void DigSystem()
    {
        if (this.selectedTile)
        {
            // Always check it in case of player move without change its look direction
            this.digTime += Time.deltaTime;

            if (this.digTime >= 0.2f)
            {
                // Todo replace with tool speed
                this.digTime = 0f;
    ...
    private void ManagePickaxeDetection()
    {
    ...
    private void ManageAxeSystemDetection()
    {
    }

    private void ManageHammerDetection()
    {
    ...
    private void OnInteractButtonReleased(InputAction.CallbackContext ctx)
    {
        ...
        else if (this.currentToolType == ToolType.HAMMER && this.currentHoveredItem)
        {
    ...
    {
        return CommonService.ArrayNotContains(this.notAllowedItemIds, WorldManager.objectsMap[posX, posY + 1]);
    }
    ...
    private bool IsDestroyableItem(Item item, Direction directionFromOrigin)
    {
        for (int i = 0; i < this.allowedItemsIds.Length; i++)
        {
            ...
            {
                if (item.GetConfig().GetId() == 16 && item.GetComponent<ItemRotation>().GetCollisionSide() !=
                    (Direction) Enum.Parse(typeof(ReverseDirection), directionFromOrigin.ToString()))
                {
                    return false;
                }
